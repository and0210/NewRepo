...
ctx.strokeStyle="rgba(255,255,255,"+(1-dist/170)/2+")";
...
function makeHeart(){
    ...
    for(let i=0;i<stars.length;i++){
        ...
        const t=(Math.PI*2/stars.length)*i;
    ...
    stars.forEach((star,index)=>{
        ...
        moveStar(star,heart[index].x,heart[index].y);
...
function moveStar(star,targetX,targetY){

    const speed=0.05;

    function animateMove(){

        star.x+=(targetX-star.x)*speed;
        star.y+=(targetY-star.y)*speed;

        if(

            Math.abs(targetX-star.x)>1 ||

            Math.abs(targetY-star.y)>1
